...
class RefineAgent(LLMAgent):
    ...
    def _validate_config(self, config: DictConfig) -> DictConfig:
        ...
        if not edit_file_api_key:
            ...
            try:
                ...
                if 'edit_file' in include_list:
                    ...
                    logger.warning('[refine] edit_file_config.api_key not set, removing edit_file from tools')
            except Exception:
                pass
        else:
        ...
        try:
            edgeone_token = (
                config.get('tools', {}).get('edgeone-pages-mcp', {}).get('env', {}).get('EDGEONE_PAGES_API_TOKEN')
            )
        except Exception:
            pass
        ...
        if not edgeone_token:
            ...
            try:
                if 'edgeone-pages-mcp' in config.get('tools', {}):
                    ...
                    logger.warning('[refine] EDGEONE_PAGES_API_TOKEN not set, removing edgeone-pages-mcp from tools')
            except Exception:
                pass
        else:
            logger.info(f'[refine] EDGEONE_PAGES_API_TOKEN is configured: {edgeone_token[:10]}...')
    ...
    async def after_tool_call(self, messages: List[Message]):
        ...
        if self.runtime.should_stop:
            if not sys.stdin.isatty():
                ...
                logger.info('[refine] Agent completed initial refinement. Waiting for user feedback.')
                ...
                try:
                    ...
                    if query:
                        logger.info(f'[refine] Received input from WebUI: {query}')
                        messages.append(Message(role='user', content=query))
                        self.runtime.should_stop = False
                        return
                    else:
                        logger.warning('[refine] Received empty input, continuing to wait...')
                        return
