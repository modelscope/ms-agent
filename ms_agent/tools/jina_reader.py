...
from ms_agent.tools.fetch_playwright_fallback import looks_like_spa_shell_html, try_playwright_inner_text
from ms_agent.utils.logger import get_logger

logger = get_logger()

DEFAULT_HEADERS: Dict[str, str] = {
    'User-Agent': 'Mozilla/5.0 (compatible; ms-agent/1.0; +https://example.com)',
...
def _build_reader_url(target_url: str, base_endpoint: str) -> str:
    ...
    base = base_endpoint if base_endpoint.endswith('/') else f'{base_endpoint}/'
...
def _should_try_playwright_after_direct(plain: str, raw_html: str, min_chars: int) -> bool:
...
def _fetch_via_jina(url: str, config: JinaReaderConfig) -> str:
    ...
    while True:
        ...
        try:
            ...
            if status in (429, 500, 502, 503, 504) and attempt <= config.retries:
                sleep_s = min(config.backoff_max, config.backoff_base * (2 ** (attempt - 1)))
...
def fetch_single_text_with_meta(url: str, config: JinaReaderConfig) -> Tuple[str, Dict[str, Any]]:
    """
    ...
    d_timeout = (
        float(config.timeout) if float(config.direct_fetch_timeout or 0) <= 0 else float(config.direct_fetch_timeout)
    )
    ...
    try_playwright = (
        ...
        and _is_direct_http_allowed(url)
        and _should_try_playwright_after_direct(direct_text, raw_html, config.playwright_retry_min_chars)
    )
    ...
    if try_playwright:
        ...
        if pw_text.strip():
            logger.info(f'Using headless Chromium fallback after Jina/direct HTTP (url prefix): {url[:80]}')
            return pw_text, {'content_source': 'playwright_fallback'}

    if direct_text:
        logger.info(f'Jina Reader returned no body for URL; using direct HTTP fallback (url prefix): {url[:80]}')
...
async def fetch_texts_via_jina(
    ...
    async def _bound(u: str) -> str:
        async with local_sem:
            return await loop.run_in_executor(executor, fetch_single_text, u, cfg)
