...
class ModelQualityChecker(ReportQualityChecker):
    ...
    def __init__(self, config: DictConfig):
        ...
        self._api_key: Optional[str] = getattr(qc_cfg, 'openai_api_key', None) or getattr(
            config.llm, 'openai_api_key', None
        )
        self._base_url: Optional[str] = getattr(qc_cfg, 'openai_base_url', None) or getattr(
    ...
    async def check(self, content: str, lang: str) -> Optional[str]:
        ...
        usr_template = self._USER_TEMPLATES.get(lang, self._USER_TEMPLATES['en'])
        ...
        try:
            ...
            logger.info(f'ModelQualityChecker ({self._model}): raw response: {raw}')
            ...
            logger.warning(f'ModelQualityChecker: failed to parse JSON from model response: {raw!r}')
            return None
...
def build_quality_checkers(config: DictConfig) -> List[ReportQualityChecker]:
    ...
    logger.info(f'Quality checker chain initialised with {len(checkers)} checker(s).')
    return checkers
