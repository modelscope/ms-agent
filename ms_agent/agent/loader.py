...
class AgentLoader:
        ...
        **kwargs,
        ...
        if code_file is not None:
            ...
            if agent_type == LLMAgent.AGENT_NAME.lower():
                agent_instance = LLMAgent(agent_config, agent_tag, trust_remote_code, **kwargs)
    ...
    def _load_external_code(cls, config, code_file, **kwargs) -> 'Agent':
        ...
        module_classes = {name: agent_cls for name, agent_cls in inspect.getmembers(code_module, inspect.isclass)}
        agent_instance = None
        for name, agent_cls in module_classes.items():
            if Agent in agent_cls.__mro__[1:] and agent_cls.__module__ == code_file:
                agent_instance = agent_cls(config, config.tag, trust_remote_code=config.trust_remote_code, **kwargs)
                break
