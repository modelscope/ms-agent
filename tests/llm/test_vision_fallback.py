...
class TestResolveSupportsVision(unittest.TestCase):
    ...
    def test_unset_falls_through_to_provider_capability(self):
        from omegaconf import OmegaConf
        from ms_agent.llm.spec import get_registry
        cfg = OmegaConf.create({'llm': {'model': 'x'}})
        # dashscope declares vision; anthropic does too; a spec-less provider
        # yields False rather than guessing.
        self.assertTrue(
            V.resolve_supports_vision(cfg, spec=get_registry().get('dashscope')))
