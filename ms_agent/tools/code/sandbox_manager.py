...
class SandboxManagerFactory:
    ...
    def ensure_local_image_exists(image: str) -> bool:
        ...
        try:
            ...
            image_exists = any(image in img.tags for img in client.images.list() if img.tags)
            if image_exists:
                logger.info(f'Image exists in local Docker registry: {image}')
            else:
                logger.info(f'Image does not exist in local Docker registry: {image}')
            return image_exists
    ...
    @staticmethod
    async def create_manager(config: Union[DictConfig, dict]) -> Union['LocalSandboxManager', 'HttpSandboxManager']:
        """
        ...
        if isinstance(config, DictConfig) and hasattr(config, 'tools') and hasattr(config.tools, 'code_executor'):
            ...
            sandbox_config = config.get('tools', {}).get('code_executor', {}).get('sandbox', {}) or config.get(
                'sandbox', {}
            )
        else:
        ...
        if mode == 'local':
            ...
            logger.info(f'Created LocalSandboxManager with cleanup_interval={cleanup_interval}s')

            if image:
                try:
                    if not SandboxManagerFactory.ensure_local_image_exists(image):
                        raise ValueError(f'Image "{image}" does not exist in local Docker registry')
                except RuntimeError as e:
                    raise ValueError(f'Error checking if image exists: {e}') from e
            else:
                logger.warning('No image specified for LocalSandboxManager, using default')
            ...
            raise ValueError(f"Unknown sandbox mode: {mode}. Must be 'local' or 'http'")
