...
class ReporterCallback(Callback):
    ...
    _POST_REPORT_GUIDANCE = {
        ...
        ('\n\n---\n'
         '**[后续工作流程建议]**\n\n'
         'Reporter 已完成报告生成。请审阅返回内容的 Execution_Summary 和 Artifacts 字段以了解生成过程和产物。'
         '如果 Reporter 未正常完成或返回信息缺失，请主动检查 reports/ 目录下的产物确定后续行动。\n\n'
         '**关于 final_report.md：** Artifacts 字段通常只包含 reports/ 目录下的文件，不包含 final_report.md——'
         '系统会在 Reporter 正常完成后自动将 reports/report.md 复制为 final_report.md。审阅和编辑应优先针对 final_report.md。'
         '如果 final_report.md 意外缺失，按此路径回退：reports/report.md -> reports/draft.md -> reports/ 下其他产物。\n\n'
         '**审查原则：**\n'
         '- 充分理解报告全貌后再进行修改——基于局部认知的编辑容易在已读和未读的章节之间引入矛盾。'
         '全文阅读一次后，后续核查使用定点方式（search_file_content 或带行号的 read_file），反复全量读取同一文件会迅速耗尽上下文预算导致任务失败。\n'
         '- 仅对具体问题（事实错误、逻辑缺陷、证据不一致、格式违规等）进行修改，不要为"润色"而改动质量良好的内容。\n'
         '- 优先使用定点修改（search_file_content -> replace_file_contents / replace_file_lines）。'
         '仅在定点修改完全无法解决时使用 write_file 重写全文，且必须完整保留所有有价值的内容，严禁使用占位符、省略标记或引用其他文件内容来替代正文。\n'
         '- 质量良好且无实质性问题时直接进入结论阶段，不要为"润色"而重写。\n'),
        'en':
        ('\n\n---\n'
         '**[Post-Report Workflow Guidance]**\n\n'
         'The Reporter has finished generating the report. Review the Execution_Summary and Artifacts fields '
         'to understand the generation process and output artifacts. '
         'If Reporter did not complete normally, check the reports/ directory to determine next steps.\n\n'
         '**About final_report.md:** Artifacts typically lists only files under reports/ '
         '(e.g., reports/report.md), not final_report.md - the system automatically copies '
         'reports/report.md to final_report.md after normal completion. '
         'Your review and edits should primarily target final_report.md. '
         'If it is unexpectedly missing, fall back to: '
         'reports/report.md -> reports/draft.md -> other artifacts under reports/.\n\n'
         '**Review principles:**\n'
         '- Familiarize yourself with the entire report before making any edits — '
         'changes based on partial knowledge commonly introduce contradictions between sections. '
         'After the initial read, use targeted approaches (search_file_content, read_file with line ranges) '
         'for subsequent checks. Repeated full reads rapidly exhaust context capacity and risk task failure.\n'
         '- Only edit to fix concrete problems (factual errors, logical flaws, evidence mismatches, '
         'style violations, etc.). Do not alter well-structured content merely for polish.\n'
         '- Prefer targeted fixes (search_file_content -> replace_file_contents / replace_file_lines). '
         'Use write_file to rewrite the full report only as a last resort, and preserve ALL valuable content in full — '
         'never use placeholders, ellipsis markers, or references to other files as substitutes for actual text.\n'
