    ...
    async def run(
        ...
        if isinstance(inputs, list):
            if isinstance(inputs[0], str):
                refractory_inputs = [[Message(role='user', content=item)] for item in inputs]  # multiple parent nodes
                ...
                raise ValueError(f"Invalid input type: List[{type(inputs[0]) if inputs else 'empty list'}]")
        ...
        for sub_inputs in refractory_inputs:
            ...
            report_type = (
                ...
                if 'analysis' in report_path
                else '**Online Sentiment Analysis Report**'
            )
        ...
        if os.path.exists(plan_path):
            try:
                ...
                logger.warning(f'Failed to load plan.json: {e}. Using empty plan.')
