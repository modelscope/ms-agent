...
from ms_agent.tools.findata.data_source_base import DataSourceError, FinancialDataSource
...
class HybridDataSource(FinancialDataSource):
    ...
    def __init__(self):
        ...
        logger.info('Hybrid data source initialized (A-shares: BaoStock, Others: AKShare)')
    ...
    def _get_source(self, code: str, market: str = None) -> List[FinancialDataSource]:
    ...
    def _call_sources(self, sources: List[FinancialDataSource], query_func: Callable) -> pd.DataFrame:
        ...
        for source in sources:
            try:
                ...
                logger.warning(f'Data source {source.__class__.__name__} failed, continue to next source: {e}')
                continue
    ...
    def get_historical_k_data(
        ...
        return self._call_sources(
            sources, lambda s: s.get_historical_k_data(code, start_date, end_date, frequency, adjust_flag, fields)
        )

    def get_stock_basic_info(self, code: str) -> pd.DataFrame:
        """Get stock basic information"""
        sources = self._get_source(code)
        return self._call_sources(sources, lambda s: s.get_stock_basic_info(code))
    ...
    def get_dividend_data(self, code: str, year: Optional[str] = None, year_type: str = 'report') -> pd.DataFrame:
        ...
        sources = self._get_source(code)
        return self._call_sources(sources, lambda s: s.get_dividend_data(code, year, year_type))
    ...
    def get_adjust_factor_data(self, code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        sources = self._get_source(code)
        return self._call_sources(sources, lambda s: s.get_adjust_factor_data(code, start_date, end_date))
    ...
    def get_financial_data(self, code: str, year: str, quarter: int, data_types: List[str]) -> Dict[str, pd.DataFrame]:
        ...
        sources = self._get_source(code)
        return self._call_sources(sources, lambda s: s.get_financial_data(code, year, quarter, data_types))
    ...
    def get_report(
        ...
        sources = self._get_source(code)
        return self._call_sources(sources, lambda s: s.get_report(code, start_date, end_date, report_type))

    def get_stock_industry(self, code: str, date: str) -> pd.DataFrame:
        """Get industry classification (BaoStock only)"""
        sources = self._get_source(code)
        return self._call_sources(sources, lambda s: s.get_stock_industry(code, date))
    ...
    def get_stock_list(self, date: str, data_type: str = 'all_a_share') -> pd.DataFrame:
        ...
        return self._call_sources(sources, lambda s: s.get_stock_list(date, data_type))

    def get_trade_dates(self, start_date: str, end_date: str) -> pd.DataFrame:
        """Get trading calendar (BaoStock only)"""
        sources = self._get_source('', market='a_share')
        return self._call_sources(sources, lambda s: s.get_trade_dates(start_date, end_date))
    ...
    def get_macro_data(
        ...
        sources = self._get_source('', market='a_share')
        return self._call_sources(sources, lambda s: s.get_macro_data(start_date, end_date, data_types, extra_kwargs))
