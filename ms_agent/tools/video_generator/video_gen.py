...
class VideoGenerator(ToolBase):
    def __init__(self, config):
        ...
        self.temp_dir = os.path.join(self.output_dir, '.temp', 'video_generator')
        ...
        return {
            'video_generator': [
                Tool(
                    ...
                    description='Generate a video with a positive prompt, and return the video file path.',
                    parameters={
                        'type': 'object',
                        'properties': {
                            'positive_prompt': {'type': 'string', 'description': 'The prompt to generate the image.'},
                            'seconds': {
                                'type': 'integer',
                                'description': 'The generated video seconds, supported is 4/8/12',
                            },
                        },
