...
class DefaultMemory(Memory):
    ...
    def __init__(self,
        ...
        self.conversation_id: Optional[str] = conversation_id or getattr(config.memory, 'conversation_id', None)
        self.persist: Optional[bool] = persist or getattr(config.memory, 'persist', None)
        self.compress: Optional[bool] = getattr(config.memory, 'compress', None)
        self.embedder: Optional[str] = getattr(config.memory, 'embedder', None)
        self.is_retrieve: Optional[bool] = getattr(config.memory, 'is_retrieve', None)
        self.path: Optional[str] = path or getattr(config.memory, 'path', None)
        self.history_mode = history_mode or getattr(config.memory, 'history_mode', None)
    ...
    def _find_messages_common_prefix(self,
            messages: List[Dict],
        ...
        filtered_cache_messages, cache_indices = _ignore_role(self.cache_messages)
        ...
        for idx in range(min_length):
            ...
            # 比较其他字段（除了忽略的字段）
            all_keys = set(current_cache_msg.keys()).union(set(current_msg.keys()))
            for key in all_keys:
    ...
    def run(self, messages, ignore_role = None, ignore_fields = None):
        print(f'ahahahah?1 : {self.memory.get_all(user_id=self.conversation_id)}')
        ...
        relevant_memories = self.memory.search(messages[-1]['content'], user_id=self.conversation_id, limit=3)
        ...
        new_messages = [{'role': 'system', 'content': system_prompt}] + messages[messages_idx:]
    ...
    def _init_memory(self) -> Mem0Memory | None:
        ...
        if self.compress:
            ...
            if llm_config is not None:
                ...
                openai_base_url = getattr(llm_config, f'{provider}_base_url', None)
                openai_api_key = getattr(llm_config, f'{provider}_api_key', None)
        ...
        mem0_config = {
            ...
            "llm": llm,
            "custom_fact_extraction_prompt": getattr(self.config.memory, 'fact_retrieval_prompt', FACT_RETRIEVAL_PROMPT),
            "vector_store": {
                "provider": "qdrant",
                "config": {
                    "path": self.path,
...
async def main():
    ...
    memory = DefaultMemory(config, path='./output', cache_messages=data, history_mode='add')
    res = memory.run(messages = [{'role': 'user', 'content': '使用bun会对新项目的影响大吗，有哪些新特性'}])
    print(res)
