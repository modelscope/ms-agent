...
class DSVideoGenerator:
    ...
    async def _generate_video(base_url, api_key, model, prompt, size, seconds):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f'{base_url}{create_endpoint}', headers=headers, json=payload) as resp:
                ...
                return await DSVideoGenerator._poll_video_task(session, base_url, task_id, headers)
    ...
    async def _poll_video_task(session, base_url, task_id, headers):
        ...
        while elapsed_time < max_wait_time:
            ...
            async with session.get(f'{base_url}{poll_endpoint}', headers=headers) as result:
                result.raise_for_status()
                data = await result.json()
                status = data['output']['task_status']
                logger.info(f'Task {task_id} status: {status}, detailed message: {str(data)}')
