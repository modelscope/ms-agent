...
class SearchAgent(CodeAgent):
    ...
    def __init__(self, config: DictConfig, tag: str, trust_remote_code: bool = False, **kwargs):
        ...
        if isinstance(self.config, DictConfig):
            if hasattr(self.config, 'llm'):
                ...
                api_key = getattr(llm_config, 'openai_api_key', '') or os.getenv('OPENAI_API_KEY')
                base_url = getattr(llm_config, 'openai_base_url', '') or os.getenv('OPENAI_BASE_URL')
                model = getattr(llm_config, 'model', '') or 'Qwen/Qwen3-235B-A22B-Instruct-2507'
                self.chat_client = OpenAIChat(api_key=api_key, base_url=base_url, model=model)
            else:
                raise ValueError('LLM configuration not found, SearchAgent requires OpenAI compatible API.')

            if hasattr(self.config, 'tools') and hasattr(self.config.tools, 'search_engine'):
                self.search_engine = get_web_search_tool(
                    config_file=getattr(self.config.tools.search_engine, 'config_file', '')
                )
            else:
            ...
            self.report_prefix = getattr(self.config, 'report_prefix', 'sentiment_')
    ...
    async def run(self, inputs: Union[str, List[Message]], **kwargs) -> List[Message]:
        ...
        if isinstance(inputs, list):
            ...
            for message in inputs[::-1]:
                if message.role == 'assistant':
                    instruction = json.loads(extract_code_blocks(message.content)[0][0].get('code', {}))
                    break
            ...
            raise ValueError('Invalid input type, SearchAgent requires a string or list of messages.')
        ...
        result_content = json.dumps({'report_path': report_path}, ensure_ascii=False, indent=2)
