...
class TestRunner:
    ...
    def run_tests(self,
        ...
        try:
            ...
            parsed_errors = self._parse_pytest_output(result.stdout,
                                                      result.stderr)
            ...
            return {
                ...
                'stdout': '',
                'stderr':
                f'Tests execution timed out after {self.timeout} seconds',
                'parsed_errors': []
            }
    ...
    def run_tests_with_feedback(
            self,
            work_dir: Path,
            error_log_path: Optional[Path] = None) -> Dict[str, Any]:
        """
        运行测试并生成反馈信息用于修复
        ...
        feedback = {
            ...
            'error_summary':
            self._generate_error_summary(result['parsed_errors'])
        }
        ...
        if error_log_path and result['parsed_errors']:
            with open(error_log_path, 'w', encoding='utf-8') as f:
                json.dump(
                    result['parsed_errors'], f, indent=2, ensure_ascii=False)

        return feedback

    def _generate_error_summary(self, parsed_errors: List[Dict[str,
                                                               Any]]) -> str:
        """
        生成错误摘要，用于指导代码修复
