...
class LlamaIndexRAG(RAG):
    ...
    def __init__(self, config: DictConfig):
        ...
        self.embedding_model = getattr(config.rag, 'embedding', 'Qwen/Qwen3-Embedding-0.6B')
        ...
        if self.retrieve_only:
            ...
            class MSCustomLLM(CustomLLM):
                ...
                def complete(_self, prompt: str, **kwargs) -> CompletionResponse:
                    message: Message = self._llm_instance.generate(
                        messages=[Message(role='user', content=prompt)], stream=False, **kwargs
                    )
                    return CompletionResponse(text=message.content)

                @llm_completion_callback()
                def stream_complete(_self, prompt: str, formatted: bool = False, **kwargs: Any):
                    for message in self._llm_instance.generate(
                        messages=[Message(role='user', content=prompt)], stream=True, **kwargs
                    ):
    ...
    def _setup_embedding_model(self, config: DictConfig):
        ...
        try:
            ...
            Settings.embed_model = HuggingFaceEmbedding(model_name=self.embedding_model, device='cpu')
    ...
    async def _retrieve(self, query: str, limit: int = 5, score_threshold: float = 0.0, **filters) -> List[dict]:
        if self.index is None:
            return []
        ...
        retriever = VectorIndexRetriever(index=self.index, similarity_top_k=limit)
    ...
    async def retrieve(self, query: str, limit: int = 5, score_threshold: float = 0.0, **filters) -> List[dict]:
        if self.retrieve_only:
        ...
        postprocessor = SimilarityPostprocessor(similarity_cutoff=score_threshold)
    ...
    async def hybrid_search(self, query: str, top_k: int = 5) -> List[dict]:
        ...
        if not bm25_available:
            ...
            try:
                bm25_retriever = BM25Retriever.from_defaults(docstore=self.index.docstore, similarity_top_k=top_k)

                fusion_retriever = QueryFusionRetriever(
                    retrievers=[vector_retriever, bm25_retriever], similarity_top_k=top_k, num_queries=1
                )
