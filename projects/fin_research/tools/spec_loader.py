...
import json
import os
from spec_constant import PRINCIPLE_ROUTING_GUIDE, PRINCIPLE_SPEC_GUIDE, WRITING_ROUTING_GUIDE, WRITING_SPEC_GUIDE
...
class SpecLoader(ToolBase):
    ...
    def __init__(self, config):
        ...
        spec_cfg = getattr(tools_cfg, 'spec_loader', None) if tools_cfg is not None else None
        self.exclude_func(spec_cfg)

        configured_dir = getattr(spec_cfg, 'spec_dir', None) if spec_cfg is not None else None
    ...
    async def get_tools(self) -> Dict[str, Any]:
        ...
        if hasattr(self, 'exclude_functions') and self.exclude_functions:
            tools['spec_loader'] = [t for t in tools['spec_loader'] if t['tool_name'] not in self.exclude_functions]
    ...
    async def load_writing_specs(self, writing_specs: List[str], **kwargs) -> str:
        writing_spec_map = self._build_writing_spec_index()
        return await self.load_specs(writing_spec_map, writing_specs, **kwargs)

    async def load_principle_specs(self, principles: List[str], **kwargs) -> str:
    ...
    async def load_specs(
        ...
        if not specs:
            return json.dumps(
                {'success': False, 'error': 'No specs provided.'},
                ensure_ascii=False,
                indent=2,
            )
        ...
        if unknown and strict:
            return json.dumps(
                {
                    'success': False,
                    'error': 'Unknown specs (strict mode): ' + ', '.join(unknown),
                },
                ensure_ascii=False,
                indent=2,
            )
        ...
        if not loaded:
            return json.dumps(
                {'success': False, 'error': 'Failed to load any specs.'},
                ensure_ascii=False,
                indent=2,
            )
        ...
        if unknown and not strict:
            sections.append(f'> Note: Unknown specs ignored: {", ".join(unknown)}')

        return json.dumps({'success': True, 'sections': join_with.join(sections)}, ensure_ascii=False, indent=2)
    ...
    def _build_writing_spec_index(self) -> Dict[str, Tuple[str, str]]:
        ...
        entries = [
            ...
            (
                ['methods', 'methodology', 'framework exposure', 'methodology exposure'],
                'writing_specs/Methodology_Exposure.md',
                'Methodology Exposure',
            ),
            (
                ['bullets', 'bullet', 'bullets & paragraphs', 'paragraph rhythm'],
    ...
    def _build_principle_spec_index(self) -> Dict[str, Tuple[str, str]]:
        ...
        entries: List[Tuple[List[str], str, str]] = [
            # synonyms, filename, canonical title
            (['mece', 'mutually exclusive and collectively exhaustive'], 'principle_specs/MECE.md', 'MECE'),
            (
                ['pyramid', 'minto', 'minto pyramid', 'pyramid principle', 'minto_pyramid'],
                'principle_specs/Minto_Pyramid.md',
                'Pyramid (Minto Pyramid)',
            ),
            (['swot', 'swot analysis'], 'principle_specs/SWOT.md', 'SWOT'),
            (['value chain', 'value-chain', 'value_chain'], 'principle_specs/Value_Chain.md', 'Value Chain'),
            (
                ['pareto', '80-20', '80/20', 'pareto 80-20', 'pareto_80-20', '8020'],
                'principle_specs/Pareto_80-20.md',
                'Pareto (80/20 Rule)',
            ),
            (
                ['boston matrix', 'bcg', 'boston consulting group', 'boston_matrix', 'boston'],
                'principle_specs/Boston_Matrix.md',
                'Boston Matrix (BCG)',
            ),
        ]
