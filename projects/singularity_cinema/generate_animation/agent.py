...
class GenerateAnimation(CodeAgent):
    ...
    async def execute_code(self, messages, **kwargs):
        ...
        if engine == 'manim':
            ...
            sys.path.pop(0)
            agent = GenerateManimCode(self.config, self.tag, self.trust_remote_code, **kwargs)
            ...
            sys.path.pop(0)
            agent = GenerateRemotionCode(self.config, self.tag, self.trust_remote_code, **kwargs)
            return await agent.execute_code(messages, **kwargs)
        else:
