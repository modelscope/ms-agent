...
async def _update_deep_research_status(session_id: str, event: Dict[str, Any]) -> None:
...
async def _send_deep_research_event(session_id: str, event: Dict[str, Any]) -> None:
...
async def websocket_session(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        is_deep_research = bool(session and session.get('project_id') == 'deep_research_v2')
...
async def handle_session_message(session_id: str, data: Dict[str, Any], websocket: WebSocket):
...
async def start_agent(session_id: str, data: Dict[str, Any], websocket: WebSocket):
    ...
    if session_type == 'chat':
        ...
        if hasattr(ms_agent, '__path__') and ms_agent.__path__:
            ...
            raise RuntimeError('Cannot determine ms_agent package path. Please ensure ms_agent is properly installed.')
        ...
        if project['id'] == 'code_genesis':
            ...
            if os.path.exists(output_dir):
                try:
                    ...
                    print(f'[Agent] WARNING: Failed to clean output directory: {e}')
    ...
    print(
        f"[Agent] Project: {project['id']}, type: {project['type']}, "
        f"config: {project['config_file']}, workflow_type: {workflow_type}"
    )

    query = data.get('query', '')
    print(f'[Agent] Query: {query[:100]}...' if len(query) > 100 else f'[Agent] Query: {query}')
    ...
    if project['id'] == 'deep_research_v2':
        try:
            ...
            await connection_manager.send_to_session(session_id, {'type': 'status', 'status': 'running'})
        except Exception as e:
            await connection_manager.send_to_session(
                session_id, {'type': 'error', 'message': f'Worker 启动失败: {str(e)}'}
            )
            session_manager.update_session(session_id, {'status': 'error'})
        return
...
async def stop_agent(session_id: str):
    ...
    await connection_manager.send_to_session(session_id, {'type': 'status', 'status': 'stopped'})
...
async def send_input(session_id: str, data: Dict[str, Any]):
    ...
    if runner.process and runner.process.returncode is not None:
        print(f'[WS] ERROR: Process has exited with code {runner.process.returncode}')
        await connection_manager.send_to_session(
            session_id,
            {
                'type': 'error',
                'message': 'Agent process has terminated. The workflow completed. Please start a new conversation to continue.',
            },
        )
        # Clean up the runner
        del agent_runners[session_id]
    ...
    try:
        ...
        await connection_manager.send_to_session(
            session_id,
            {'type': 'error', 'message': f'Failed to send input: {str(e)}. The process may have terminated.'},
        )
...
async def send_status(session_id: str, websocket: WebSocket):
    ...
    if session:
        await websocket.send_json(
            {'type': 'status', 'session': session, 'messages': session_manager.get_messages(session_id)}
        )
...
async def on_agent_output(session_id: str, message: Dict[str, Any]):
    ...
    if msg_type == 'stream':
        # Streaming update
        await connection_manager.send_to_session(
            session_id, {'type': 'stream', 'content': content, 'done': message.get('done', False)}
        )
        if message.get('done'):
            session_manager.add_message(session_id, role, content, 'text')
    else:
        session_manager.add_message(session_id, role, content, msg_type, message.get('metadata'))
        await connection_manager.send_to_session(
            session_id,
            {
                'type': 'message',
...
async def on_agent_log(session_id: str, log: Dict[str, Any]):
    """Handle agent log"""
    await connection_manager.send_to_session(session_id, {'type': 'log', **log})
...
async def on_agent_progress(session_id: str, progress: Dict[str, Any]):
    ...
    await connection_manager.send_to_session(session_id, {'type': 'progress', **progress})
...
async def on_agent_complete(session_id: str, result: Dict[str, Any]):
    ...
    await connection_manager.send_to_session(session_id, {'type': 'complete', 'result': result})


async def on_agent_error(session_id: str, error: Dict[str, Any]):
    """Handle agent error"""
    session_manager.update_session(session_id, {'status': 'error'})
    session_manager.add_message(session_id, 'system', error.get('message', 'Unknown error'), 'error')
    ...
    await connection_manager.send_to_session(session_id, {'type': 'error', **error})
