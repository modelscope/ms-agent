...
class RenderRemotion(CodeAgent):
    def __init__(self, config: DictConfig, tag: str, trust_remote_code: bool = False, **kwargs):
        ...
        self.render_immediate_fix = getattr(self.config, 'render_immediate_fix', True)
        ...
        self.max_visual_fix_rounds = getattr(self.config, 'max_visual_fix_rounds', 2)
    ...
    async def execute_code(self, messages: Union[str, List[Message]], **kwargs) -> List[Message]:
        ...
        subprocess.run('npm install', cwd=self.remotion_project_dir, shell=True, check=True)

        segment_status = {
            i: os.path.exists(os.path.join(self.render_dir, f'scene_{i + 1}', f'Scene{i + 1}.mov'))
            for i in range(len(segments))
        }

        for round_idx in range(self.code_fix_round + 1):
            # Identify segments needing render (all initially, then only failed ones)
            segments_to_render = [i for i, status in segment_status.items() if status is not True]

            if not segments_to_render:
                logger.info('All segments rendered successfully.')
                break

            logger.info(f'Round {round_idx + 1}: Rendering {len(segments_to_render)} segments...')

            results = {}

            def _read_current_code(seg_i: int) -> str:
                code_path = os.path.join(self.remotion_code_dir, f'Segment{seg_i + 1}.tsx')
                if os.path.exists(code_path):
                    with open(code_path, 'r', encoding='utf-8') as f:
                        return f.read()
                project_code_path = os.path.join(self.remotion_project_dir, 'src', f'Segment{seg_i + 1}.tsx')
                if os.path.exists(project_code_path):
                    with open(project_code_path, 'r', encoding='utf-8') as f:
                        return f.read()
                return ''

            def _extract_error_segment_indices(log_text: Optional[str]) -> List[int]:
                if not log_text:
                    return []
            ...
            for i in segments_to_render:
                ...
                to_fix = sorted({idx for idx in to_fix if 0 <= idx < len(segments)})
                ...
                for fix_i in to_fix:
                    ...
                    _, fixed_code = self._fix_code_static(
                        fix_i, err_text, current_code, self.config, self.remotion_project_dir
                    )
                    if fixed_code:
                        self._update_segment_code(fix_i, fixed_code)
                        # If we fixed a different segment, we should probably reset its status too
                        if fix_i != i:
                            segment_status[fix_i] = False  # Force re-render of the culprit later if it was skipped
    ...
    def _update_segment_code(self, i, code):
        ...
        dst_file = os.path.join(self.remotion_project_dir, 'src', f'Segment{i + 1}.tsx')
        with open(dst_file, 'w', encoding='utf-8') as f:
            f.write(code)

    def _setup_remotion_project(self, segments, audio_infos):
        # 1. Create project structure
        os.makedirs(os.path.join(self.remotion_project_dir, 'src'), exist_ok=True)
        os.makedirs(os.path.join(self.remotion_project_dir, 'public', 'images'), exist_ok=True)
        ...
        os.makedirs(os.path.join(self.remotion_project_dir, 'src', 'images'), exist_ok=True)

        if os.path.exists(self.images_dir):
            for file in os.listdir(self.images_dir):
                src = os.path.join(self.images_dir, file)
                dst_public = os.path.join(self.remotion_project_dir, 'public', 'images', file)
                dst_src = os.path.join(self.remotion_project_dir, 'src', 'images', file)
        ...
        with open(os.path.join(self.remotion_project_dir, 'package.json'), 'w') as f:
            json.dump(package_json, f, indent=2)

        # 5. Create src/index.ts
        with open(os.path.join(self.remotion_project_dir, 'src', 'index.ts'), 'w') as f:
    ...
    def _generate_root_tsx(self, segments, audio_infos):
        ...
        with open(os.path.join(self.remotion_project_dir, 'src', 'Root.tsx'), 'w') as f:
    ...
    def _update_root_tsx_for_segment(self, segment_idx):
        ...
        with open(os.path.join(self.remotion_project_dir, 'tsconfig.json'), 'w') as f:
            json.dump(tsconfig, f, indent=2)

    def _ensure_browser(self, remotion_project_dir):
        # Check for global browser cache to avoid re-downloading.
        user_home = os.path.expanduser('~')
        global_browser_cache = os.path.join(user_home, '.ms_agent_remotion_browser')
        local_browser_cache = os.path.join(remotion_project_dir, 'node_modules', '.remotion')

        # Link or copy cached browser if available.
        if os.path.exists(global_browser_cache):
            logger.info(f'Found cached Chrome in {global_browser_cache}. Linking to project...')
        ...
        if browser_found:
            logger.info("Remotion browser detected locally. Skipping 'browser ensure'.")
            # Cache it for next time!
            if not os.path.exists(global_browser_cache):
                try:
                    logger.info(f'Caching browser to {global_browser_cache} for future runs...')
        ...
        logger.info('Attempting to manually download Remotion browser from npmmirror...')
        ...
        try:
            ...
            logger.warning(f'Failed to manually download browser from mirror: {e}')
        ...
        if os.name == 'nt':
            remotion_cmd = os.path.abspath(os.path.join(remotion_project_dir, 'node_modules', '.bin', 'remotion.cmd'))
        else:
            remotion_cmd = os.path.abspath(os.path.join(remotion_project_dir, 'node_modules', '.bin', 'remotion'))
        ...
        try:
            if os.name == 'nt' and 'remotion.cmd' in remotion_cmd:
                ...
                subprocess.run(cmd_str, cwd=remotion_project_dir, shell=True, check=True)
            else:
                cmd = [remotion_cmd, 'browser', 'ensure']
                if isinstance(remotion_cmd, str) and ' ' in remotion_cmd and 'npx' in remotion_cmd:
            ...
            logger.warning(f'Failed to download browser from mirror via CLI: {e}')
        ...
        if system_chrome:
            logger.info(f'System Chrome detected at {system_chrome}. Configuring Remotion to use it.')
    ...
    def _render_remotion_scene_static(
        i, segment, duration, config, work_dir, render_dir, remotion_project_dir, mllm_check_round=0, scale=0.9
        ...
        output_path = os.path.abspath(os.path.join(output_dir_scene, f'Scene{i + 1}.mov'))
        ...
        remotion_cache_dir = os.path.join(remotion_project_dir, 'node_modules', '.remotion')

        # 1. Check Local Cache
        if os.path.exists(remotion_cache_dir):
            for root, _, files in os.walk(remotion_cache_dir):
                if 'chrome-headless-shell.exe' in files:
                    browser_executable = os.path.abspath(os.path.join(root, 'chrome-headless-shell.exe'))
                    break
                elif 'chrome-headless-shell' in files:
                    browser_executable = os.path.abspath(os.path.join(root, 'chrome-headless-shell'))
                    break
        ...
        if browser_executable:
            ...
            # Add stability flags
            base_cmd.extend(
                ['--chromium-options', 'no-sandbox,disable-setuid-sandbox,disable-gpu,disable-dev-shm-usage']
            )
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                logger.warning(f'Rendering failed for {composition_id}. Log (except): {log_content[:500]}...')
    ...
    def _extract_preview_frames_static(video_path, segment_id, work_dir):
        ...
        for frame_idx, timestamp in timestamps.items():
            output_path = os.path.join(test_dir, f'segment_{segment_id + 1}_{frame_idx}.png')
    ...
    @staticmethod
    def _fix_code_static(i, error_log, code, config, remotion_project_dir=None):
        ...
        return i, RenderRemotion._fix_code_impl(llm, error_log, code, remotion_project_dir)

    @staticmethod
    def _fix_code_impl(llm, error_log, code, remotion_project_dir=None):
        available_images_info = ''
        if remotion_project_dir:
            images_path = os.path.join(remotion_project_dir, 'public', 'images')
