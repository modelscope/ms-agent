...
def project_help_text():
    ...
    if projects:
        return f'Built-in bundled project name under package ms_agent/projects. Available: {", ".join(projects)}'
...
class RunCMD(CLICommand):
    ...
    def define_args(parsers: argparse.ArgumentParser):
        ...
        parser.add_argument(
            ...
            default=None,
            metavar='PATH',
            help='Path to a .env file. If omitted, loads ./.env from the current '
        ...
        parser.add_argument(
            '--config', required=False, type=str, default=None, help='The directory or the repo id of the config file'
        )
        parser.add_argument(
            '--project',
            required=False,
        ...
        parser.add_argument('--mcp_config', required=False, type=str, default=None, help='The extra mcp server config')
        parser.add_argument(
            '--mcp_server_file', required=False, type=str, default=None, help='An extra mcp server file.'
        )
        parser.add_argument(
            '--openai_api_key',
            required=False,
        ...
        if getattr(self.args, 'knowledge_search_paths', None):
            ...
            if paths:
                ...
                if tl is None or not OmegaConf.is_config(tl):
                    ...
                    config.tools['localsearch'] = OmegaConf.create(localsearch_config)
                else:
