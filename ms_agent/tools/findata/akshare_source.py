...
class AKShareDataSource(FinancialDataSource):
        ...
        try:
            ...
            if code.startswith('sh.') or code.startswith('sz.') or code.startswith('bj'):
                ...
                df = akshare.stock_zh_a_hist(
                    symbol=clean_code, period=period, start_date=st_date, end_date=ed_date, adjust=adjust
                )
    ...
    def _get_hk_basic_info(self, code: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.warning(f'Failed to fetch HK stock business info for {code}')
            ...
            result_df = pd.DataFrame(
                {
                    'code': [code],
                    'code_name': [
                        df_base_info.loc[df_base_info['item'] == '股票简称', 'value'].iloc[0]
                        if not df_base_info.loc[df_base_info['item'] == '股票简称', 'value'].empty
                        else ''
                    ],
                    'listingDate': [
                        df_base_info.loc[df_base_info['item'] == '上市时间', 'value'].iloc[0]
                        if not df_base_info.loc[df_base_info['item'] == '上市时间', 'value'].empty
                        else ''
                    ],
                    'outDate': [''],
                    'type': ['1'],
    ...
    def get_dividend_data(self, code: str, year: Optional[str] = None, year_type: str = 'report') -> pd.DataFrame:
        ...
        raise DataSourceError('get_dividend_data is not supported by AKShareDataSource; use BaoStock or Hybrid')
    ...
    def get_adjust_factor_data(self, code: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        raise DataSourceError('get_adjust_factor_data is not supported by AKShareDataSource; use BaoStock or Hybrid')

    def get_financial_data(self, code: str, year: str, quarter: int, data_types: List[str]) -> dict:
        """
        ...
        if code.startswith(('hk.', 'us.')):
            ...
            clean_code = self._convert_code(code, market='HK' if code.startswith('hk.') else 'US')
        ...
        def _select_row_by_report(df: pd.DataFrame) -> pd.DataFrame:
            ...
            if 'REPORT_DATE' in d.columns:
                d['_dt'] = pd.to_datetime(d['REPORT_DATE']).dt.date.astype('str')
            ...
            for col in ('REPORT_DATE_NAME', 'REPORT_TYPE'):
                if col in d.columns:
                    hit = d[d[col].astype(str).str.contains(str(year)) & d[col].astype(str).str.contains(target_qname)]
                    if not hit.empty:
                        return hit
            ...
            if 'REPORT_DATE' in d.columns:
                d['_dt'] = pd.to_datetime(d['REPORT_DATE']).dt.date.astype('str')
                d['_diff'] = (pd.to_datetime(d['_dt']) - pd.to_datetime(target_date)).abs()
                d = d.sort_values('_diff')
                return d.drop(columns=['_dt', '_diff']).head(1)

            return d.head(1)

        META_KEEP = ['REPORT_DATE', 'REPORT_TYPE', 'REPORT_DATE_NAME', 'NOTICE_DATE', 'UPDATE_DATE']

        def _filter_columns(row_df: pd.DataFrame, category: str) -> pd.DataFrame:
            if row_df.empty:
                return row_df
            ...
            keep = set(c for c in cols if any(re.match(p, c) for p in PATTERNS[category]))
            keep |= set(c for c in META_KEEP if c in cols)

            out = row_df.loc[:, [c for c in row_df.columns if c in keep]].copy()
            ...
            if category == 'dupont' and 'XSJLL' not in out.columns:
                ...
                if base_cols.issubset(set(row_df.columns)):
                    try:
                        ...
                        logger.warning(f'Failed to calculate XSJLL_calc for {code}: {e}')
        ...
        if code.startswith(('hk.', 'us.')):
            try:
                ind_df = (
                    ...
                    if code.startswith('hk.')
                    else akshare.stock_financial_us_analysis_indicator_em(symbol=clean_code)
                )
            ...
            needs_indicator = any(dt in ('profit', 'operation', 'growth', 'dupont') for dt in data_types)
            ...
            if needs_indicator:
                try:
                    ...
                    logger.warning(f'Failed to fetch financial_analysis_indicator: {e}')
                    ind_df = pd.DataFrame()

            for data_type in data_types:
                try:
                    result[data_type] = pd.DataFrame()
                    if data_type in ('profit', 'operation', 'growth', 'dupont'):
                        if ind_df.empty:
                            logger.warning(f'No indicator row for {code} {year}Q{quarter}')
                            continue
                        result[data_type] = _filter_columns(ind_df, data_type)
                        continue

                    elif data_type == 'balance':
                        df = akshare.stock_balance_sheet_by_report_em(symbol=code.replace('.', '').upper())
                        row = _select_row_by_report(df)
                        if not row.empty:
                            result[data_type] = row

                    elif data_type == 'cash_flow':
                        df = akshare.stock_cash_flow_sheet_by_report_em(symbol=code.replace('.', '').upper())
                        row = _select_row_by_report(df)
                        if not row.empty:
                            result[data_type] = row
    ...
    def get_report(
        ...
        raise DataSourceError('get_report is not supported by AKShareDataSource; use BaoStock or Hybrid')

    def get_stock_industry(self, code: str, date: str) -> pd.DataFrame:
        """Industry classification is not supported by AKShare."""
        raise DataSourceError('get_stock_industry is not supported by AKShareDataSource; use BaoStock or Hybrid')
    ...
    def get_stock_list(self, date: str, data_type: str = 'all_a_share') -> pd.DataFrame:
        ...
        logger.info(f'Fetching stock list for {data_type}, only support a_share and latest data')
    ...
    def get_macro_data(
        ...
        logger.info(f'Fetching macroeconomic data ({start_date} to {end_date}) {data_types}')
        ...
        for data_type in data_types:
            try:
                if data_type in ('deposit_rate', 'loan_rate'):
                    ...
                    raise DataSourceError('Required reserve ratio is not supported by AKShare')
                elif data_type == 'money_supply_year':
                    result[data_type] = self._get_money_supply_data_year(start_date, end_date)
                elif data_type == 'money_supply_month':
                    result[data_type] = self._get_money_supply_data_month(start_date, end_date)
                else:
    ...
    def _get_money_supply_data_month(
        ...
        try:
            ...
            df['月份'] = pd.to_datetime(df['月份'].str.replace('月份', '').str.replace('年', '-'))
            df['月份'] = df['月份'].dt.to_period('M')
            if start_date:
                df = df[df['月份'] >= pd.to_datetime(start_date).strftime('%Y-%m')]
            if end_date:
                df = df[df['月份'] <= pd.to_datetime(end_date).strftime('%Y-%m')]

            return df.sort_values('月份').reset_index(drop=True)
        except Exception as e:
            raise DataSourceError(f'Error fetching monthly money supply data: {e}')
    ...
    def _get_money_supply_data_year(
        ...
        last_in_year = month_df.sort_values('月份').groupby('年', as_index=False).tail(1).reset_index(drop=True)
        cols = [
            '货币和准货币(M2)-数量(亿元)',
            '货币和准货币(M2)-同比增长',
            '货币(M1)-数量(亿元)',
        ...
        year_df = last_in_year[['年'] + [c for c in cols if c in last_in_year.columns]]
