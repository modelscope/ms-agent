...
class GenerateVideo(CodeAgent):
    ...
    async def execute_code(self, messages: Union[str, List[Message]], **kwargs) -> List[Message]:
        ...
        for i in range(len(segments)):
            if 'video' in segments[i]:
                with open(os.path.join(self.video_prompts_dir, f'segment_{i + 1}.txt'), 'r') as f:
                    video_prompts.append(f.read())
            else:
                video_prompts.append(None)
        logger.info('Generating videos.')

        tasks = [(i, segment, prompt) for i, (segment, prompt) in enumerate(zip(segments, video_prompts))]

        # Use ThreadPoolExecutor for parallel execution
        with ThreadPoolExecutor(max_workers=self.num_parallel) as executor:
            futures = [
                executor.submit(self._process_single_video_static, i, segment, prompt, self.config, self.videos_dir)
    ...
    def _process_single_video_static(i, segment, prompt, config, videos_dir):
        ...
        try:
            loop.run_until_complete(GenerateVideo._process_single_video_impl(i, segment, prompt, config, videos_dir))
        finally:
            loop.close()

    @staticmethod
    async def _process_single_video_impl(i, segment, prompt, config, videos_dir):
        if prompt is None:
