...
class GenerateImages(CodeAgent):
    ...
    async def execute_code(self, messages: Union[str, List[Message]], **kwargs) -> List[Message]:
        ...
        for i in range(len(segments)):
            illustration_path = os.path.join(self.illustration_prompts_dir, f'segment_{i + 1}.txt')
            if self.config.background == 'image' and os.path.exists(illustration_path):
        ...
        with ThreadPoolExecutor(max_workers=self.num_parallel) as executor:
            ...
            for i, segment, prompt_text in tasks:
                ...
                futures.append(
                    executor.submit(
                        self._process_single_illustration_static, i, segment, final_prompt, self.config, self.images_dir
                    )
                )
    ...
    @staticmethod
    def _process_single_illustration_static(i, segment, prompt, config, images_dir):
        ...
        try:
            loop.run_until_complete(
                GenerateImages._process_single_illustration_impl(i, segment, prompt, config, images_dir)
            )
            loop.run_until_complete(
                GenerateImages._process_foreground_illustration_impl(i, segment, config, images_dir)
            )
        finally:
            loop.close()

    @staticmethod
    async def _process_single_illustration_impl(i, segment, prompt, config, images_dir):
        ...
        if config.background != 'image':
            ...
            logger.info(f'Generating image. Prompt: {prompt[:50]}... kwargs: {kwargs}')
            ...
            if not _temp_file or not os.path.exists(_temp_file):
                logger.error(f'Background image generation failed for segment {i + 1}. Result: {_temp_file}')
                return
    ...
    @staticmethod
    async def _process_foreground_illustration_impl(i, segment, config, images_dir):
        ...
        illustration_prompts_dir = os.path.join(work_dir, 'illustration_prompts')
        ...
        for idx, _req in enumerate(foreground_assets):
            ...
            prompt = re.sub(r'<think>.*?</think>', '', prompt_text, flags=re.DOTALL).strip()
    ...
    def fade(input_image, output_image, segment, fade_factor=0.3, brightness_boost=80, opacity=1.0):
        ...
        if has_animation:
            logger.info('Applying fade effect to background image (Animation present)')
