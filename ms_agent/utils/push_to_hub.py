...
from ms_agent.utils.utils import get_files_from_dir, is_package_installed, text_hash
...
class PushToGitHub(PushToHub):
    ...
    def _create_github_repo(
        ...
        payload = {'name': repo_name, 'description': description, 'private': visibility == 'private', 'auto_init': True}
        ...
        if response.status_code == 201:
            ...
            if 'name already exists' in error_message:
                logger.info(f"Repository '{repo_name}' already exists. Will attempt to upload files to it.")
                return None
            else:
    ...
    def _upload_files(
        ...
        for full_path in files_to_upload:
            ...
            blobs.append({'path': remote_path_str, 'mode': '100644', 'type': 'blob', 'sha': response.json()['sha']})
...
class PushToModelScope(PushToHub):
    ...
    def __init__(
        ...
        if not is_package_installed('modelscope'):
            raise ImportError('ModelScope package is not installed. Please install it with `pip install modelscope`.')
    ...
    @staticmethod
    def _preprocess(folder_path: str, path_in_repo_url: str, add_powered_by: bool = True) -> 'Tuple[str, str]':
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                if add_powered_by and not content.lstrip().startswith('<span style='):
                    content = (
                        """<span style="color: darkgreen; font-weight: bold; font-family: monospace;
            ...
            if count > 0:
                with open(file_path, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                logger.info(f"Preprocessed {count} 'resources/' links in {file_path}.")
            else:
                logger.info(f'No "resources/" links found in {file_path}. No changes made.')

        except IOError as e:
            logger.error(f'Error reading or writing the report file: {e}')
            return '', ''
        except Exception as e:
            logger.error(f'Unexpected error during preprocessing of PushToModelScope: {e}')
            return '', ''
    ...
    def _postprocess(report_file_path: str, report_file_path_in_cache: str):
        try:
            ...
            shutil.rmtree(os.path.dirname(report_file_path_in_cache), ignore_errors=True)
        except FileNotFoundError:
            logger.warning(f'The backup file of report: {report_file_path_in_cache} does not exist.')
    ...
    def push(
        ...
        *,
        ...
        try:
            ...
            logger.info(f'Successfully pushed files to ModelScope: {target_url}')
        except Exception as e:
    ...
    def push(
        ...
        *,
        ...
        if repo_type not in ['model', 'dataset']:
            raise ValueError("Repository type must be either 'model' or 'dataset'.")
