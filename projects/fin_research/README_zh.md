...
- **多维度分析**：融合金融数据指标与公众舆情两个维度，产出覆盖面广、结构清晰的研究报告。
...
2. **Searcher Agent（搜索智能体）**：依据编排智能体的舆情研究任务，调用深度研究工作流（`ms-agent/projects/deep_research`）进行舆情深度检索与研究，生成舆情分析报告。

3. **Collector Agent（数据采集智能体）**：依据编排智能体的金融数据分析任务，使用基于 `akshare`/`baostock` 实现的数据获取工具采集所需金融数据。
