...
class CreateBackground(CodeAgent):
    ...
    async def execute_code(self, messages, **kwargs):
        ...
        for line in title_lines:
            ...
            draw.text((config['padding'], y_position), line, font=title_font, fill=slogan_subtitle_color)
        ...
        for i, line in enumerate(subtitle_lines):
            ...
            draw.text((x_offset, y_position), line, font=subtitle_font, fill=slogan_subtitle_color)
            y_position += bbox[3] - bbox[1] + 5

        line_y = height - config['padding'] - config['line_position_offset']
        if self.config.use_subtitle:
            draw.line([(0, line_y), (width, line_y)], fill=slogan_subtitle_color, width=config['line_width'])
