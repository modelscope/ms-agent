...
class OpenAIChat:
    ...
    def chat(self, messages: List[Dict[str, Any]], tools: List[Dict[str, Any]] = None, **kwargs) -> Dict[str, Any]:
        ...
        res_d: Dict[str, Any] = dict(
            ...
            finish_reason=completion.choices[0].finish_reason,  # 'stop', 'tool_calls', 'length', None
            usage=completion.usage.to_dict(),
        )

        return res_d

    def chat_stream(self, messages: List[Dict[str, Any]], tools: List[Dict[str, Any]] = None, **kwargs):
        """
        ...
        assert kwargs.get('stream', True), "Streaming must be enabled by setting 'stream=True' in kwargs."
        ...
        for chunk in completion:
            ...
            res_d['reasoning_content'] = delta.reasoning_content if hasattr(delta, 'reasoning_content') else ''
            res_d['content'] = delta.content
            res_d['tool_calls'] = delta.tool_calls if hasattr(delta, 'tool_calls') else []
    ...
    @staticmethod
    def aggregate_stream_chunks(stream_chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for chunk_d in stream_chunks:
            ...
            res_d['reasoning_content'] += (
                chunk_d.get('reasoning_content', '') if chunk_d.get('reasoning_content') is not None else ''
            )
            res_d['content'] += chunk_d.get('content', '') if chunk_d.get('content') is not None else ''

            if chunk_d.get('tool_calls') is not None:
                res_d['tool_calls'].extend(chunk_d.get('tool_calls', []))
            res_d['finish_reason'] = chunk_d.get('finish_reason', res_d['finish_reason'])

            # Get the last usage information as final usage for current round (consider cache tokens)
            if chunk_d.get('usage') is not None:
                res_d['usage']['completion_tokens'] = chunk_d['usage'].get('completion_tokens', 0)
                res_d['usage']['prompt_tokens'] = chunk_d['usage'].get('prompt_tokens', 0)
                res_d['usage']['total_tokens'] = chunk_d['usage'].get('total_tokens', 0)

        return res_d

    @staticmethod
    def convert_message(role: Literal['assistant', 'tool'], round_message: Dict[str, Any]) -> Dict[str, Any]:
        if role == 'assistant':
            ...
            for tool_call in round_message['tool_calls']:
                if isinstance(tool_call, ChoiceDeltaToolCall):
                    ...
                    tool_call = tool_call.model_dump(include=['id', 'index', 'type', 'function'])
                else:
                    raise ValueError(f'Unsupported tool call type: {type(tool_call)}. Expected ChoiceDeltaToolCall.')
            ...
            # TODO: tbd ...
            raise ValueError('`tool message` is to be implemented in the future.')

        else:
            raise ValueError(f"Unsupported role: {role}. Supported roles are 'assistant' and 'tool' for now.")
    ...
    def chat_stream_mt(
        ...
        **kwargs,
        ...
        if 'system' not in roles:
            system_message: Dict[str, Any] = {'role': 'system', 'content': 'You are a helpful assistant.'}
            messages.insert(0, system_message)

        assert len(messages) >= 2, 'At least two messages are required: user and system'
        ...
        while True:
            try:
                ...
                round_d: Dict[str, Any] = self.aggregate_stream_chunks(streaming_chunks)
                yield round_d

                # Convert `round_d` to OpenAI's chat messages format
                ## Assistant Message
                if round_d['role'] == 'assistant':
                    assistant_message = self.convert_message(role='assistant', round_message=round_d)
                    ...
                    for tool_call in assistant_message.get('tool_calls', []):
                        if tool_call['type'] == 'function':
                            ...
                            tool_call_result = available_functions[function_name](**function_args)
                ...
                if round_d['finish_reason'] in ['stop', 'tool_calls', 'length']:
                    break
        ...
        if history[-1]['role'] == 'tool':
            messages = history + [{'role': 'user', 'content': 'Please output the tool calling results very briefly.'}]
            round_item: dict = self.aggregate_stream_chunks(
                [chunk_item for chunk_item in self.chat_stream(messages=messages, tools=tools, **kwargs)]
            )
