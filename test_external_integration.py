...
def test_deep_research_caller():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = caller.run(query='什么是人工智能？', output_dir=output_dir)
...
def test_code_scratch_caller():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = caller.run(
            prompt="创建一个简单的Python程序，打印'Hello World'", work_dir=work_dir)

        print(f"Code Scratch 调用结果: {result['success']}")
        print(f"返回码: {result['returncode']}")
        if result['stderr']:
            error_preview = result['stderr'][:200] if len(
                result['stderr']) > 200 else result['stderr']
