...
class AgentRunner:
    ...
    def __init__(
        ...
        self._is_chat_mode = project.get('id') == '__chat__'  # Simple chat mode flag
    ...
    async def send_input(self, text: str):
        ...
        if not self.process:
            ...
            if self.on_error:
                self.on_error(
                    {'message': 'Agent process is not running. Please start a new conversation.', 'type': 'input_error'}
                )
            return

        # Check if process has exited
        if self.process.returncode is not None:
            print(f'[Runner] ERROR: Process has exited with code {self.process.returncode}, cannot send input')
            if self.on_error:
                self.on_error(
                    {'message': 'Agent process has terminated. Please start a new conversation.', 'type': 'input_error'}
                )
            return
        ...
        if not self.process.stdin:
            ...
            if self.on_error:
                self.on_error({'message': 'Cannot send input: process stdin is not available.', 'type': 'input_error'})
            return
        ...
        try:
            ...
            if self.on_error:
                self.on_error(
                    {
                        'message': f'Failed to send input: Process may have terminated. Error: {str(e)}',
                        'type': 'input_error',
                    }
                )
    ...
    def _build_command(self, query: str) -> list:
        ...
        if workflow_type == 'simple' and project_type == 'workflow':
            ...
            simple_config_file = os.path.join(project_path, 'simple_workflow.yaml')
        ...
        if project_type == 'workflow' or project_type == 'agent':
            ...
            cmd = ['ms-agent', 'run', '--config', config_file, '--trust_remote_code', 'true']
            ...
            if llm_config.get('api_key'):
                ...
                if provider == 'modelscope':
                    cmd.extend(['--llm.modelscope_api_key', llm_config['api_key']])
                    ...
                    if llm_config.get('base_url'):
                        cmd.extend(['--llm.modelscope_base_url', llm_config['base_url']])
                    # Pass model if set by user
                    if llm_config.get('model'):
                        cmd.extend(['--llm.model', llm_config['model']])
                    # Pass temperature if set by user (in generation_config)
                    if temperature_enabled and llm_config.get('temperature') is not None:
                        cmd.extend(['--generation_config.temperature', str(llm_config['temperature'])])
                    # Pass max_tokens if set by user (in generation_config)
                    if llm_config.get('max_tokens'):
                        cmd.extend(['--generation_config.max_tokens', str(llm_config['max_tokens'])])
                    ...
                    # Pass base_url if set by user
                    if llm_config.get('base_url'):
                        cmd.extend(['--llm.openai_base_url', llm_config['base_url']])
                    # Pass model if set by user
                    if llm_config.get('model'):
                        cmd.extend(['--llm.model', llm_config['model']])
                    # Pass temperature if set by user (in generation_config)
            ...
            if self.project.get('id') != '__chat__':
                ...
                if edit_file_config.get('api_key'):
                    ...
                    cmd.extend(['--tools.file_system.edit_file_config.api_key', edit_file_config['api_key']])
                    if edit_file_config.get('base_url'):
                        cmd.extend(['--tools.file_system.edit_file_config.base_url', edit_file_config['base_url']])
                    if edit_file_config.get('diff_model'):
                        cmd.extend(['--tools.file_system.edit_file_config.diff_model', edit_file_config['diff_model']])
                else:
                    ...
                    try:
                        ...
                        if config_data and 'tools' in config_data and 'file_system' in config_data['tools']:
                            include_list = config_data['tools']['file_system'].get('include', [])
                            if isinstance(include_list, list) and 'edit_file' in include_list:
                                # Remove edit_file from the list
                                filtered_include = [tool for tool in include_list if tool != 'edit_file']
                                # Pass the filtered list as comma-separated string
                                cmd.extend(['--tools.file_system.include', ','.join(filtered_include)])
                    except Exception as e:
                        print(f'[Runner] Warning: Could not read config file to exclude edit_file: {e}')
                ...
                if edgeone_pages_config.get('api_token'):
                    ...
                    cmd.extend(
                        ['--tools.edgeone-pages-mcp.env.EDGEONE_PAGES_API_TOKEN', edgeone_pages_config['api_token']]
                    )
        ...
        try:
            ...
            while (self.is_running or process_exited) and self.process:
                ...
                if self.process.returncode is not None and not process_exited:
                    process_exited = True
                    print(f'[Runner] Process exited with code: {self.process.returncode}')
                        ...
                        if self.process.returncode is None:
                            ...
                            if self._is_chat_mode:
                                ...
                                if self.on_output and not self._waiting_input_sent:
                                    ...
                                    self._waiting_input_sent = True
                            print('[Runner] Agent is waiting for user input, keeping process alive...')
                            # Keep process alive and wait for input
                            await asyncio.sleep(0.5)  # Small delay to avoid busy waiting
                            continue
                        else:
                            print('[Runner] Process exited while waiting for input')
                ...
                print(f'[Runner] Output: {text[:200]}' if len(text) > 200 else f'[Runner] Output: {text}')
                try:
            ...
            if self.process:
                ...
                if self._collecting_assistant_output and self._accumulated_output.strip():
                    cleaned = re.sub(r'\[INFO:ms_agent\]\s*', '', self._accumulated_output.strip())
                    cleaned = re.sub(r'\[([^\]]+)\]\s*', '', cleaned, count=1)
                    print(f'[Runner] Flushing accumulated output on process exit: {cleaned[:200]}...')
                        ...
                        self.on_output(
                            {
                                ...
                                'content': cleaned,
                                'role': 'assistant',
                                'metadata': {'agent': self._current_step or 'agent'},
                            }
                        )
                    self._accumulated_output = ''
                    self._collecting_assistant_output = False
                    ...
                    self.on_output(
                        {
                            ...
                            'role': 'assistant',
                            'metadata': {'step': self._current_step, 'status': 'completed'},
                        }
                    )
                ...
                if self._waiting_for_input:
                    ...
                    if return_code == 0:
                        ...
                        if self.on_output and not self._waiting_input_sent:
                            ...
                            self._waiting_input_sent = True
                        if self.on_complete:
                            self.on_complete({'status': 'success', 'message': 'Agent completed successfully'})
                    else:
                        if self.on_error:
                            self.on_error(
                                {
                                    'message': (
                                        f'Agent process terminated while waiting for input. Exit code: {return_code}'
                                    ),
    ...
    async def _process_chat_line(self, line: str):
        ...
        if self._collecting_tool_call:
            if cleaned:
                ...
                if cleaned == '}' and self._tool_call_json_buffer.strip().startswith('{'):
        ...
        if 'execute tool call' in line:
            if self.on_output:
                ...
                self.on_output(
                    {'type': 'tool_result', 'content': cleaned, 'role': 'assistant', 'metadata': {'is_error': is_error}}
                )
            return
    ...
    def _flush_tool_call(self):
        ...
        if self._is_chat_mode and self._tool_call_json_buffer.strip() and self.on_output:
            try:
                import json
    ...
    def _flush_chat_response(self):
        ...
        if self._is_chat_mode and self._chat_response_buffer.strip() and self.on_output:
            print(f'[Runner] Chat complete: {len(self._chat_response_buffer)} chars')
            self.on_output(
                {'type': 'stream', 'content': self._chat_response_buffer.strip(), 'role': 'assistant', 'done': True}
            )
            self._chat_response_buffer = ''
    ...
    async def _process_line(self, line: str):
        ...
        if self.on_log:
            ...
            await self.on_log(
                {
                    'level': log_level,
                    'message': cleaned_message if cleaned_message else line,
                    'timestamp': datetime.now().isoformat(),
                }
            )
    ...
    def _scan_and_send_output_files(self, programmer_step=None):
        ...
        try:
            ...
            print(f'[Runner] Found {len(generated_files)} files in tasks.txt: {generated_files}')
                ...
                self.on_output(
                    {
                        ...
                        'role': 'assistant',
                        'metadata': {'files': generated_files, 'source': 'tasks.txt'},
                    }
                )
    ...
    async def _detect_patterns(self, line: str):
        ...
        if not url_match:
            url_match = re.search(r'(https?://mcp\.edgeone\.site/[^\s]+)', line)
        ...
        if 'openai.' in line.lower() and ('error' in line.lower() or 'Error' in line):
            ...
            if json_match:
                try:
                    ...
                    if 'error' in error_data and 'message' in error_data['error']:
                        ...
                        error_type = error_data['error'].get('type', 'API Error')
                    ...
                    self.on_error(
                        {
                            ...
                            'type': 'api_error',
                            'code': error_match.group(1) if error_match.groups() else None,
                        }
                    )
                # Also send as output message so it appears in the conversation
                if self.on_output:
        ...
        begin_match = re.search(r'\[([^\]]+)\]\s*Agent\s+\S+\s+task\s+beginning', line)
        if begin_match:
            step_name = begin_match.group(1)

            # Skip sub-steps and programmer agents (handled separately)
            if ('-r' in step_name and '-' in step_name.split('-r')[-1]) or step_name.startswith('programmer-'):
                return
                    ...
                    self.on_output(
                        {
                            ...
                            'content': cleaned,
                            'role': 'assistant',
                            'metadata': {'agent': self._current_step},
                        }
                    )
                self._accumulated_output = ''
                self._collecting_assistant_output = False
            ...
            step_status = {
                s: (
                    'completed'
                    if i < self._workflow_steps.index(step_name)
                    else 'running'
                    if s == step_name
                    else 'pending'
                )
                ...
                self.on_output(
                    {
                        ...
                        'content': step_name,
                        'role': 'assistant',
                        'metadata': {'step': step_name, 'status': 'running'},
                    }
                )
        ...
        if programmer_match:
            ...
            if not self._current_step or not self._current_step.startswith('programmer-'):
                print(f'[Runner] First programmer agent detected: {programmer_agent} - starting coding step')
                ...
                step_status = {
                    s: (
                        'completed'
                        if i < self._workflow_steps.index('coding')
                        else 'running'
                        if s == 'coding'
                        else 'pending'
                    )
                    ...
                    self.on_output(
                        {
                            ...
                            'content': 'coding',
                            'role': 'assistant',
                            'metadata': {'step': 'coding', 'status': 'running'},
                        }
                    )
        ...
        def flush_accumulated_output():
            print(
                f'[Runner] flush_accumulated_output called: '
            ...
            print(
                ...
                if len(self._accumulated_output) > 200
                else f'[Runner] Buffer content: {self._accumulated_output}'
            )
            if self._collecting_assistant_output and self._accumulated_output.strip():
                # Clean log prefixes
                cleaned_content = re.sub(r'\[INFO:ms_agent\]\s*', '', self._accumulated_output.strip())
                cleaned_content = re.sub(r'\[([^\]]+)\]\s*', '', cleaned_content, count=1)
                print(f'[Runner] Flushing assistant output: {cleaned_content[:100]}...')
        ...
        if end_match:
            ...
            for s in self._workflow_steps:
                step_status[s] = (
                    'completed' if self._workflow_steps.index(s) <= self._workflow_steps.index(step_name) else 'pending'
                )
                ...
                self.on_output(
                    {
                        ...
                        'content': step_name,
                        'role': 'assistant',
                        'metadata': {'step': step_name, 'status': 'completed'},
                    }
                )
        ...
        if '[assistant]:' in line:
            in_coding = self._current_step and self._current_step.startswith('programmer-')
        ...
        # Continue collecting assistant output
            ...
            if self._current_step and self._current_step.startswith('programmer-'):
                self._collecting_assistant_output = False
                self._accumulated_output = ''
                # Don't return - continue processing
            else:
                # Check if new pattern starts
                if '[tool_calling]:' in line or ('[assistant]:' in line and 'Agent' not in line):
                    if self._accumulated_output.strip():
                        cleaned = self._clean_log_prefix(self._accumulated_output.strip())
                        if cleaned and self.on_output:
                            self.on_output(
                                {
                                    'type': 'agent_output',
                    ...
                    if line.strip():
                        ...
                        if cleaned_line:
                            ...
                            url_match = re.search(r'(https?://[^\s]*edgeone\.cool[^\s]*)', cleaned_line)
                            if url_match:
                                deployment_url = url_match.group(1)
                                # Clean up escaped characters in URL (e.g., \& -> &)
                                deployment_url = deployment_url.replace('\\&', '&')
                                print(f'[Runner] Detected deployment URL in assistant: {deployment_url}')
                                if self.on_output:
                                    self.on_output(
                                        {
                                            'type': 'deployment_url',
                            ...
                            if 'Waiting for user feedback' in line or 'Waiting for user input from stdin' in line:
        ...
        if '[tool_calling]:' in line:
            ...
            if len(parts) > 1:
                ...
                if json_part.startswith('{'):
                    ...
                    tool_match = re.search(r'([\w\-]+(?:---[\w\-]+)?)', json_part)
                    if tool_match:
        ...
        if self._collecting_tool_call:
            ...
            if '[INFO:ms_agent]' in line:
                agent_match = re.search(r'\[INFO:ms_agent\]\s*\[([^\]]+)\]', line)
                if agent_match:
            ...
            if (
                ...
                try:
                    ...
                    if tool_name and tool_name != 'unknown':
                        ...
                        print(f'[Runner] Sending tool call: {tool_name}, agent: {agent_name}')
                        if self.on_output:
                            self.on_output(
                                {
                                    'type': 'tool_call',
                                    ...
                                    'role': 'assistant',
                                    'metadata': {'tool_name': tool_name, 'tool_args': tool_args, 'agent': agent_name},
                                }
                            )
                        ...
                        print(f'[Runner] WARNING: Invalid tool_name: {tool_name}')
                    ...
                    if '"tool_name"' in self._tool_call_json_buffer:
                        print(f'[Runner] JSON incomplete, continuing... (error: {str(e)[:50]})')
                except Exception as e:
            ...
            if '[assistant]:' in line or 'Agent' in line and 'task' in line or '[tool_result]:' in line:
                ...
                if self._tool_call_json_buffer:
                    tool_name_match = re.search(r'"tool_name"\s*:\s*"([^"]+)"', self._tool_call_json_buffer)
                    if tool_name_match:
                        tool_name = tool_name_match.group(1)
                        # Try to extract arguments - handle nested JSON objects
                        args_start = self._tool_call_json_buffer.find('"arguments"')
                        tool_args = {}
                        if args_start != -1:
                            brace_start = self._tool_call_json_buffer.find('{', args_start)
                            if brace_start != -1:
                                brace_count = 0
                                brace_end = brace_start
                                for i in range(brace_start, len(self._tool_call_json_buffer)):
                                ...
                                if brace_end > brace_start:
                                    args_str = self._tool_call_json_buffer[brace_start:brace_end]
                                    try:
        ...
        if self._collecting_tool_result:
            ...
            if line.strip() and not line.strip().startswith('['):
                ...
                url_match = re.search(r'"url":\s*"(https?://[^"]+edgeone\.(cool|site)[^"]+)"', line)
                # Pattern 2: Direct URL with edgeone.cool or edgeone.site
                if not url_match:
                    url_match = re.search(r'(https?://[^\s]*edgeone\.(cool|site)[^\s]*)', line)
                if url_match:
                    deployment_url = url_match.group(1)
                    # Clean up escaped characters in URL (e.g., \& -> &)
                    deployment_url = deployment_url.replace('\\&', '&')
                    ...
                    if self.on_output:
                        ...
                        if not self._waiting_input_sent:
                            self.on_output(
                                {
                                    'type': 'waiting_input',
                                    'content': 'You can now provide additional feedback or visit the deployed site.',
                                    'role': 'system',
                                    'metadata': {'waiting': True, 'deployment_complete': True},
                                }
                            )
                            self._waiting_input_sent = True

                # Send result if we have tool name and accumulated enough content
                if self._current_tool_name and len(self._current_tool_result) > 100 and self.on_output:
                    self.on_output(
                        {
                            'type': 'tool_result',
                            'content': f'工具 {self._current_tool_name} 执行完成',
        ...
        # Detect file writing
        file_match = re.search(r'writing file:?\s*["\']?([^\s"\']+)["\']?', line.lower())
        if not file_match:
            file_match = re.search(r'creating file:?\s*["\']?([^\s"\']+)["\']?', line.lower())
        if file_match and self.on_progress:
            filename = file_match.group(1)
            self.on_progress({'type': 'file', 'file': filename, 'status': 'writing'})
            return

        # Detect file written/created/saved - multiple patterns
        file_keywords = ['file created', 'file written', 'file saved', 'saved to:', 'wrote to', 'generated:', 'output:']
        if any(keyword in line.lower() for keyword in file_keywords):
            ...
            file_match = re.search(r'["\']?([a-zA-Z0-9_\-][^\s"\'\/\[\]]*\.[a-zA-Z0-9]+)["\']?', line)
            if file_match and self.on_progress:
                filename = file_match.group(1)
                # Validate filename: must not be just numbers or version numbers like "0.0"
                if filename and not re.match(r'^\d+\.\d+$', filename) and len(filename) > 2:
                    ...
                    self.on_progress({'type': 'file', 'file': filename, 'status': 'completed'})
            return

        # Detect output file paths (e.g., "output/user_story.txt" standalone)
        output_path_match = re.search(r'(?:^|\s)((?:output|projects)/[^\s]+\.[a-zA-Z0-9]+)(?:\s|$)', line)
        ...
        if (
            'Initial refinement completed' in line
            ...
            if self.on_output and not self._waiting_input_sent:
                self.on_output(
                    {
                        'type': 'waiting_input',
                        'content': '✅ Initial refinement completed. You can now provide additional feedback or modifications.',
                        'role': 'system',
                        'metadata': {'waiting': True},
                    }
                )
